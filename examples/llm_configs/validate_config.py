            ...
            for provider_name, provider_config in providers.items():
                ...
                if "models" in provider_config:
                    ...
                    if not isinstance(models, dict):
                        errors.append(f"Provider '{provider_name}' models must be a dictionary")
                    else:
...
def validate_with_pydantic(config_data: dict[str, Any]) -> list[str]:
    ...
    try:
        ...
        if config.default_provider not in config.providers:
            errors.append(f"Default provider '{config.default_provider}' not found in providers")
    ...
    for provider_name, provider_config in config.providers.items():
        try:
            ...
            results[provider_name] = {
                ...
                "message": message,
                "capabilities": (handler.get_capabilities().__dict__ if is_valid else None),
            }
...
def print_validation_results(errors: list[str], warnings: list[str] | None = None) -> None:
...
def print_provider_test_results(results: dict[str, Any]) -> None:
    ...
    for provider_name, result in results.items():
        ...
        if result["capabilities"]:
            ...
            print(f"    • Supports streaming: {capabilities.get('supports_streaming', False)}")
...
def main() -> None:
    ...
    parser.add_argument("--test-providers", action="store_true", help="Test provider connections")
