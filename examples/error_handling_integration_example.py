...
async def demonstrate_provider_integration():
    ...
    try:
        ...
        for provider_name, provider_class in providers:
            ...
            logger.info(
                f"✅ {provider_name} inherits _error_handling_components from "
                f"BaseSourceControlProvider"
            )
        ...
        for provider_name in providers:
            ...
            if components:
                ...
                if "resilient_manager" in components:
                    manager_type = type(components["resilient_manager"]).__name__
                    logger.info(f"   Resilient manager: {manager_type}")
                if "circuit_breaker" in components:
                    breaker_type = type(components["circuit_breaker"]).__name__
                    logger.info(f"   Circuit breaker: {breaker_type}")
                if "fallback_manager" in components:
                    fallback_type = type(components["fallback_manager"]).__name__
                    logger.info(f"   Fallback manager: {fallback_type}")
            else:
