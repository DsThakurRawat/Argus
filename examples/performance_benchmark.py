...
class PerformanceBenchmark:
    ...
    def __init__(
        self, config_path: str = "examples/llm_configs/multi_provider_config.yaml"
    ) -> None:
    ...
    async def _benchmark_agent(
        self, agent: EnhancedTriageAgent, strategy_name: str
    ) -> dict[str, float]:
    ...
    def generate_report(self, results: dict[str, dict]) -> str:
        ...
        if "optimization_strategies" in results:
            ...
            report.append(
                f"{headers[0]:<20} {headers[1]:<15} {headers[2]:<12} "
                f"{headers[3]:<12} {headers[4]:<12}"
            )
            report.append("-" * 80)
        ...
        if "load_testing" in results:
            ...
            report.append(
                f"Min Response Time: {load_metrics.get('min_response_time_ms', 0):.2f} ms"
            )
            report.append(
