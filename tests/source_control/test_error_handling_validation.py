...
class TestErrorHandlingConfigValidator:
    ...
    def test_validate_circuit_breaker_config_invalid_thresholds(self, validator: str) -> None:
        ...
        assert any(
            "success_threshold should be less than failure_threshold" in error for error in errors
        )
    ...
    def test_validate_circuit_breaker_config_extreme_values(self, validator: str) -> None:
        ...
        is_valid, errors = validator.validate_circuit_breaker_config(config)
        assert not is_valid
        assert any("failure_threshold should not exceed 1000" in error for error in errors)
        assert any("recovery_timeout should not exceed 3600 seconds" in error for error in errors)
        assert any("success_threshold should not exceed 100" in error for error in errors)
    ...
    def test_validate_operation_circuit_breaker_config_invalid_operations(self, validator):
        ...
        assert not is_valid
        assert any(
            "file_operations: failure_threshold must be at least 1" in error for error in errors
        )
    ...
    def test_validate_retry_config_invalid_values(self, validator: str) -> None:
        ...
        assert any("backoff_factor must be greater than 1.0" in error for error in errors)
    ...
    def test_validate_retry_config_logical_errors(self, validator: str) -> None:
        ...
        is_valid, errors = validator.validate_retry_config(config)
        assert not is_valid
        assert any("base_delay should be less than max_delay" in error for error in errors)
    ...
    def test_validate_metrics_config_invalid(self, validator: str) -> None:
        ...
        assert not is_valid
        assert any("enabled must be a boolean" in error for error in errors)
        assert any("collection_interval should be at least 1 second" in error for error in errors)
        ...
        if not any("retention_period should not exceed 30 days" in error for error in errors):
            # Try with a higher value
            config["retention_period"] = 86400 * 31  # 31 days
            is_valid, errors = validator.validate_metrics_config(config)
            assert any("retention_period should not exceed 30 days" in error for error in errors)
    ...
    def test_validate_graceful_degradation_config_invalid(self, validator: str) -> None:
        ...
        assert any(
            "Strategy reduced_timeout enabled flag must be a boolean" in error for error in errors
        )
    ...
    def test_validate_error_handling_config_invalid(self, validator: str) -> None:
        ...
        assert any("health_checks: enabled must be a boolean" in error for error in errors)
