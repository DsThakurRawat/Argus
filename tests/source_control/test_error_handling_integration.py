...
class TestErrorHandlingIntegration:
    ...
    def test_get_provider_config(self) -> None:
        ...
        assert github_config["error_handling"]["circuit_breaker"]["failure_threshold"] == 5

        # Test GitLab config
        gitlab_config = get_provider_config("gitlab")
        assert "error_handling" in gitlab_config
        assert gitlab_config["error_handling"]["circuit_breaker"]["failure_threshold"] == 5

        # Test Local config
        local_config = get_provider_config("local")
        assert "error_handling" in local_config
        assert local_config["error_handling"]["circuit_breaker"]["failure_threshold"] == 10
    ...
    async def test_enhanced_github_provider_initialization(self, mock_github_config):
        ...
        with patch("github.Github") as mock_github_class, patch("github.Repository.Repository"):
    ...
    async def test_github_provider_error_handling(self, mock_github_config):
        ...
        with patch("github.Github") as mock_github_class, patch("github.Repository.Repository"):
            ...
            provider.operations.get_file_content = AsyncMock(side_effect=Exception("Network error"))
    ...
    async def test_gitlab_provider_error_handling(self, mock_gitlab_config):
        ...
        with patch("gitlab.Gitlab") as mock_gitlab_class:
            ...
            provider.file_ops.get_file_content = AsyncMock(side_effect=Exception("API error"))
    ...
    async def test_batch_operations_error_handling(self, mock_local_config):
        ...
        with (
            ...
            provider.batch_ops.batch_operations = AsyncMock(side_effect=Exception("Batch error"))
    ...
    async def test_graceful_degradation_integration(self, mock_github_config):
        ...
        with patch("github.Github") as mock_github_class, patch("github.Repository.Repository"):
            ...
            provider.graceful_degradation_manager.execute_with_graceful_degradation = AsyncMock(
    ...
    async def test_circuit_breaker_integration(self, mock_github_config):
        ...
        with patch("github.Github") as mock_github_class, patch("github.Repository.Repository"):
            ...
            provider.operations = MagicMock()
            provider.operations.get_file_content = AsyncMock(side_effect=Exception("API error"))
    ...
    async def test_metrics_integration(self, mock_github_config):
        ...
        with patch("github.Github") as mock_github_class, patch("github.Repository.Repository"):
            ...
            # Mock operations
            provider.operations = MagicMock()
            provider.operations.get_file_content = AsyncMock(return_value="file content")
