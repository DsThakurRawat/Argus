...
class TestEnhancedBaseAgent:
    ...
    async def test_execute_success(
        ...
        mock_llm_service.generate_structured = AsyncMock(
            return_value=TextResponse(text="Generated text", confidence=0.9)
        )
    ...
    async def test_execute_with_fallback(
        ...
        mock_llm_service.generate_structured = AsyncMock(
            side_effect=[
                ...
                TextResponse(text="Fallback text", confidence=0.8),
            ]
        )
        mock_llm_service_class.return_value = mock_llm_service
    ...
    def test_configuration_updates(self, mock_llm_config: str) -> None:
        ...
        with patch("argus.agents.enhanced_base.EnhancedLLMService"), patch(
            ...
            assert agent.provider_preference == [
                ProviderType.OPENAI,
                ProviderType.GEMINI,
            ]
...
class TestEnhancedAgentAdapter:
    ...
    async def test_adapter_execute_enhanced(
        ...
        mock_enhanced_agent.execute = AsyncMock(
            return_value=TextResponse(text="Enhanced response", confidence=0.9)
        )
    ...
    async def test_adapter_execute_legacy(
        ...
        legacy_agent.fallback_model = "gemini-1.5-flash"
        legacy_agent.__class__.__name__ = "TextAgent"
        legacy_agent.execute = AsyncMock(
            return_value=TextResponse(text="Legacy response", confidence=0.8)
        )
