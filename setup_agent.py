...
def setup_env(provider, api_key, base_url=None):
    ...
    new_lines = [
        ...
        and not (base_url and line.startswith(f"{provider.upper()}_BASE_URL="))
    ]
    ...
    config = {
        ...
        "providers": {
            provider: {
                ...
                "base_url": base_url,
                "models": {model: {"name": model, "model_type": "smart", "max_tokens": 4096}},
            }
        },
        "agents": {
...
def run_setup():
    ...
    console.print("[bold white]argus[/]  ·  [bold red]v2026.05.16[/]\n")

    status = Text()
    status.append(f"User: [bold white]{getpass.getuser()}[/]\n")
    status.append("System: [bold green]Ready for Configuration[/]")
    console.print(
        Panel(status, title="[bold red]Dashboard Status[/]", border_style="red", width=80)
    )
    ...
    choice = safe_input(
        "\n[bold red]> [/][bold white]Select Provider[/]",
    ...
    if provider != "ollama":
        while True:
            api_key = safe_input(
                f"[bold red]> [/][bold white]Enter {provider.upper()} API Key[/]", password=True
            )
            if not api_key:
            ...
            if len(api_key) < 20:
                ...
                if (
                    safe_input(
                        "[bold red]> [/][bold white]Continue anyway? (y/n)[/]",
                        choices=["y", "n"],
                        default="n",
                    )
                    != "y"
    ...
    if provider in ["azure", "openrouter", "ollama"]:
        base_url = safe_input(
            "[bold red]> [/][bold white]Enter Base URL[/]",
            default="http://localhost:11434" if provider == "ollama" else "",
        )
    ...
    m_choice = safe_input(
        "\n[bold red]> [/][bold white]Select Model[/]",
