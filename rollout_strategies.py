...
class RolloutManager:
    ...
    async def start_rollout(self) -> bool:
        ...
        logger.info(
            f"Starting {self.config.strategy.value} rollout with {self.config.percentage:.1%} traffic"
        )
        self.is_active = True
        self.start_time = time.time()
    ...
    async def _collect_metrics(self) -> RolloutMetrics:
        ...
        return RolloutMetrics(
            ...
            successful_requests=int(
                metrics_summary.get("total_requests", 0)
                * metrics_summary.get("overall_success_rate", 0)
            ),
            failed_requests=int(
                metrics_summary.get("total_requests", 0)
                * (1 - metrics_summary.get("overall_success_rate", 0))
            ),
    ...
    async def _evaluate_rollout_health(self, metrics: RolloutMetrics) -> bool:
        ...
        if metrics.success_rate < self.config.success_threshold:
            logger.warning(
                f"Success rate {metrics.success_rate:.2%} below threshold {self.config.success_threshold:.2%}"
            )
            return False
        ...
        if metrics.error_rate > self.config.error_threshold:
            logger.warning(
                f"Error rate {metrics.error_rate:.2%} above threshold {self.config.error_threshold:.2%}"
            )
            return False
        ...
        if metrics.avg_latency_ms > self.config.latency_threshold_ms:
            logger.warning(
                f"Latency {metrics.avg_latency_ms:.2f}ms above threshold {self.config.latency_threshold_ms:.2f}ms"
            )
            return False
        ...
        if metrics.total_cost > self.config.cost_threshold:
            logger.warning(
                f"Cost ${metrics.total_cost:.2f} above threshold ${self.config.cost_threshold:.2f}"
            )
            return False
    ...
    async def _generate_rollout_report(self, status: str):
        ...
        avg_latency = sum(m.avg_latency_ms for m in self.rollout_metrics) / len(
        ...
        print("-" * 40)
        print(
            f"Success Rate: {overall_success_rate:.2%} (threshold: {self.config.success_threshold:.2%})"
        )
        print(
