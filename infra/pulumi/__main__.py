...
log_topic_name = pulumi.Config().get("log_topic_name") or "argus-logs"
log_subscription_name = pulumi.Config().get("log_subscription_name") or "argus-logs-sub"
log_sink_name = pulumi.Config().get("log_sink_name") or "argus-log-sink"
agent_service_account_id = pulumi.Config().get("agent_service_account_id") or "argus-sa"

# --- Pub/Sub Topic for Logs ---
logs_topic = gcp.pubsub.Topic("logs-topic", project=project_id, name=log_topic_name)

# --- Pub/Sub Subscription for Logs ---
logs_subscription = gcp.pubsub.Subscription(
    "logs-subscription",
    project=project_id,
...
agent_sa = gcp.serviceaccount.Account(
    "agent-sa",
    project=project_id,
...
agent_pubsub_subscriber_binding = gcp.projects.IAMMember(
    "agent-pubsub-subscriber-binding",
    project=project_id,
    role="roles/pubsub.subscriber",
    member=agent_sa.member,
)

agent_vertex_ai_user_binding = gcp.projects.IAMMember(
    "agent-vertex-ai-user-binding",
    project=project_id,
    role="roles/aiplatform.user",
    member=agent_sa.member,
)

agent_logging_viewer_binding = gcp.projects.IAMMember(
    "agent-logging-viewer-binding",
    project=project_id,
...
# --- Cloud Logging Sink ---
logs_sink = gcp.logging.ProjectSink(
    "logs-sink",
    project=project_id,
    name=log_sink_name,
...
logs_sink_iam_member = gcp.pubsub.TopicIAMMember(
    "logs-sink-iam-member",
    topic=logs_topic.name,
