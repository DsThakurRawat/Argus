...
class TriageAgent:
    ...
    async def analyze_logs(self, logs: list[str], flow_id: str) -> TriagePacket:
        ...
        logger.debug(
            f"[TRIAGE] Prompt for triage model: flow_id={flow_id}, prompt={prompt[:500]}..."
        )
        ...
        try:
            ...
            logger.debug(
                f"[TRIAGE] Raw model response: flow_id={flow_id}, response={json_response_str[:500]}..."
            )
            ...
            logger.info(
                f"[TRIAGE] Triage analysis complete: flow_id={flow_id}, issue_id={triage_packet.issue_id}, severity={triage_packet.preliminary_severity_score}"
            )
            return triage_packet

        except ValidationError as e:
            logger.error(
                f"[ERROR_HANDLING] Failed to validate TriagePacket schema from model response: flow_id={flow_id}, error={e}"
            )
            raise ValueError(f"Invalid model response schema: {e}") from e
        except json.JSONDecodeError as e:
            logger.error(
                f"[ERROR_HANDLING] Failed to decode JSON from model response: flow_id={flow_id}, error={e}, response={json_response_str}"
            )
            raise ValueError(f"Malformed JSON response from model: {e}") from e
        except Exception as e:
            logger.error(
                f"[ERROR_HANDLING] Error calling Gemini Triage model: flow_id={flow_id}, error={e}"
            )
